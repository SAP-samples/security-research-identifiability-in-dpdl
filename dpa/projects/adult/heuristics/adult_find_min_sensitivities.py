...
X = np.load('./adult.npz')
X = np.ascontiguousarray(X['x'])#[:10000]

a = np.ascontiguousarray(np.load('./adult.npz')['x'])

X_hashed = [hash_val(xi, False) for xi in a]
X_hashed_hex = [hash_val(xi, True) for xi in a]
...
np.savez_compressed(path.join(save_filepath, f"original_data_adult_set_size_{SIZE_D}_last_place_{place+1}.npz"), lookup=X[:SIZE_D])
np.savez_compressed(path.join(save_filepath, f"alt_data_adult_set_size_{SIZE_D}_last_place_{place+1}.npz"), lookup=alt_set)