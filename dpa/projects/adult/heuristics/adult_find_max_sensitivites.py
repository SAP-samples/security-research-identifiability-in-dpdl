...
X = np.load('./adult.npz')
X = np.ascontiguousarray(X['x'][:80000])
...
save_filepath = f"./"
np.savez_compressed(path.join(save_filepath, f"original_data_set_adult_size_{SIZE_D}_place_1.npz"), lookup=X[:SIZE_D])
np.savez_compressed(path.join(save_filepath, f"alt_data_set_adult_size_{SIZE_D}_place_1.npz"), lookup=alt_set)