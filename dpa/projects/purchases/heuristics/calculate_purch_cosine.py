...
X = X['x'][:size_subsample]
data = X

a = np.load('./shokri_purchases_100_classes_pca.npz')
a = a['x'][:size_subsample]

#%
def mat_mul(start_idx: int) -> None:

    # repeat batch to work on, i.e., [1,1,1,2,2,2]
    X_first = np.repeat(a[start_idx : start_idx + batch_size], len_X, axis=0)
    #orig_X_first = np.repeat(X[start_idx : start_idx + batch_size], len_X, axis=0)
    # tile all images, i.e, [1,2,3,1,2,3]
    X_second = np.tile(a, (batch_size, 1))
    #orig_X_second = np.tile(X, (batch_size, 1))

    # Cosine similiarity 
    X_dist = np.fromiter(
        (
            np.dot(im_first, im_second)/(np.linalg.norm(im_first)*np.linalg.norm(im_second))
