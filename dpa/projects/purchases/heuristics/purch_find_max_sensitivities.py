...
filepath = os.getcwd()+f"/mnist_distances"
max_per_picture = {}
...
#        list_of_argmax.append(np.argmax(list(dicti.values())[SIZE_D:]))
#        list_of_maximums.append(max(list(dicti.values())[SIZE_D:]))
#    arg_img = np.argmax(list(list_of_maximums))
#    temp_key = list(list_of_dicts[arg_img].keys())[SIZE_D+list_of_argmax[arg_img]]
#    list_of_dicts[arg_img][temp_key] = 0
...
np.savez_compressed(path.join(save_filepath, f"original_data_set_ssim_size_{SIZE_D}_place_{place+1}.npz"), lookup=X[:SIZE_D])
np.savez_compressed(path.join(save_filepath, f"alt_data_set_ssim_size_{SIZE_D}_place_{place+1}.npz"), lookup=alt_set)
