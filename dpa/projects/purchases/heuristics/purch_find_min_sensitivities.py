...
filepath = os.getcwd()+f"/purch_ham"
min_per_picture = {}
...
save_filepath = f"./"
np.savez_compressed(path.join(save_filepath, f"min_per_picture_ham_set_size_{SIZE_D}.npz"), lookup=min_per_picture)
...
np.savez_compressed(path.join(save_filepath, f"original_data_ham_set_size_{SIZE_D}_last_place_{place+1}.npz"), lookup=X[:SIZE_D])
np.savez_compressed(path.join(save_filepath, f"alt_data_ham_set_size_{SIZE_D}_last_place_{place+1}.npz"), lookup=alt_set)