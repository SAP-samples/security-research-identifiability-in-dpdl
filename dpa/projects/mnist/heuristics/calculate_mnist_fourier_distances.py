...
def mat_mul(start_idx: int) -> None:
    ...
    X_dist = np.fromiter(
        (
            cv2.norm((20*np.log(cv2.magnitude(np.fft.fftshift(cv2.dft(np.float32(im_first),flags = cv2.DFT_COMPLEX_OUTPUT))[:,:,0],np.fft.fftshift(cv2.dft(np.float32(im_first),flags = cv2.DFT_COMPLEX_OUTPUT))[:,:,1])+1))-(20*np.log(cv2.magnitude(np.fft.fftshift(cv2.dft(np.float32(im_second),flags = cv2.DFT_COMPLEX_OUTPUT))[:,:,0],np.fft.fftshift(cv2.dft(np.float32(im_second),flags = cv2.DFT_COMPLEX_OUTPUT))[:,:,1])+1)), cv2.NORM_L2)
            for im_first, im_second in zip(X_first, X_second)
        ),
        dtype=np.float16,
        count=X_first.shape[0],
    )
