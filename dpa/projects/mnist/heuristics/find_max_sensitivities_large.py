...
save_filepath = f"./"
np.savez_compressed(path.join(save_filepath, f"max_per_picture.npz"), lookup=max_per_picture)
...
save_filepath = f"./"
np.savez_compressed(path.join(save_filepath, f"original_data.npz"), lookup=X[:SIZE_D])
np.savez_compressed(path.join(save_filepath, f"alt_data.npz"), lookup=alt_set)