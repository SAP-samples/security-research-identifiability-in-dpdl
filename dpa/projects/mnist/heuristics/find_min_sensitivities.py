...
save_filepath = f"./"
np.savez_compressed(path.join(save_filepath, f"min_per_picture_ssim_set_size_{SIZE_D}.npz"), lookup=min_per_picture)
...
np.savez_compressed(path.join(save_filepath, f"original_data_ssim_set_size_{SIZE_D}_last_place_{place+1}.npz"), lookup=X[:SIZE_D])
np.savez_compressed(path.join(save_filepath, f"alt_data_ssim_set_size_{SIZE_D}_last_place_{place+1}.npz"), lookup=alt_set)