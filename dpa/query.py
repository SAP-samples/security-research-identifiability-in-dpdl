...
class Model:
    def __init__(self, input_shape=(28, 28, 1), classes=10):
        ...
        x= Conv2D(16, kernel_size=(3, 3), activation='relu', input_shape=input_shape, data_format='channels_last')(inputs)
        x= BatchNormalization(axis=3)(x)
        x=MaxPool2D(pool_size=(2, 2))(x)
        x=Conv2D(16, kernel_size=(3, 3), activation='relu', data_format='channels_last')(x)
        x=BatchNormalization(axis=3)(x)
        x=MaxPool2D(pool_size=(2, 2))(x)
        x=Flatten()(x)
    ...
    def train(self, train_set, alternative_set, test_set, local, worst_case, bounded):
        ...
        num_samples_orig = len(x_train)
        num_samples_alt = len(x_alt)
        print("samples orig ", num_samples_orig, " samples alt ", num_samples_alt, " y train shape ", y_train.shape[1:])
        ...
        loss = tf.keras.losses.CategoricalCrossentropy(reduction=tf1.losses.Reduction.NONE)(labels, probs)
